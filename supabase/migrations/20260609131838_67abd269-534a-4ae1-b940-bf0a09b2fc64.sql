...
-- Trigger-only / admin-only functions: revoke from public roles
...
REVOKE EXECUTE ON FUNCTION public.get_admin_payments() FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.is_any_admin(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.is_email_unsubscribed(text) FROM PUBLIC, anon;

-- Authenticated-only functions
REVOKE EXECUTE ON FUNCTION public.get_opportunity_contacts(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.increment_tender_views(uuid) FROM PUBLIC, anon;

-- get_agricapital_partition is intentionally callable by anon (public dashboard)
-- Keep default execute grants.
